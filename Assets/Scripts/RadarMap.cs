    ...
    public class RadarMap : Graphic
    {
        ...
        private void DrawRadar(Vector2[] bounds)
        {
            ...
            for (int i = 0, length = polygonVertex.Length; i < length; i++)
            {
                ...
                var outDir = GetHorizontalDir(polygonVertex[i]);
        ...
        public void SetData(float[] values, bool useAnmi = false)
        {
            ...
            else
            {
                if (twnner != null)
                {
                    StopCoroutine(twnner);
                }
                twnner = StartCoroutine(StartAnim(m_Values, values));
            }
        }
        ...
        /// 获取垂直向量
        /// </summary>
        private Vector2 GetHorizontalDir(Vector2 _dir)
        {
